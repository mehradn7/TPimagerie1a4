...
%
M = [1/6 1/6 1/6 1/6 ; -1/3 -1/3 -1/3 1 ; -sqrt(2)/3 -sqrt(2)/3 2*sqrt(2)/3 0 ; sqrt(2)/sqrt(3) -sqrt(2)/sqrt(3) 0
0] ;
%
...
%
psi1d = (image_ihs(x1,y1,2)/pi)*180 ; % psi1 en degrés
...
%
image_rgb = image_rgb( :, :,1 :3) ; %image_rgb est l'image ayant subi une rotation colorimétrique GLOBALE d'angle theta0
%
...
sintheta = norm(u) ;
costheta = sum(v1.*v2) ;
THETA(nolig,nocol) = atan2(sintheta,costheta) ;% THETA(nolig,nocol) est l'angle entre la teinte de couleur à remplacer et celle de la
...
for k=1 :3
image_rgb_filtre( :, :,k) = FILTRE.*im_depart( :, :,k) + (1-FILTRE).*image_rgb( :, :,k) ; %image_rgb est maintenant l'image filtree
end
